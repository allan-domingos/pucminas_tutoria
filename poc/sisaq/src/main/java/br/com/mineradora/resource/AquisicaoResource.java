...
public class AquisicaoResource {
	...
	@RequestMapping(value = "solicitacao/{id}" , method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> findAllBySolicitacaoId(@PathVariable("id") final BigInteger id) {
        return ResponseEntity.ok(this.aquisicaoService.findAllBySolicitacaoId(id));
    }
